...
import { GenericPda } from "../pda/genericPda";
...
export const handleCreatePropertySystem  = async (
  ...
  const propertySystemAddress = address(message.accountKeys.at(instruction.accounts[1]!)!);

  const propertySystemAccount : PdaTypes.propertySystemAccountType  = await GenericPda("propertySystemAccount",propertySystemAddress)  as PdaTypes.propertySystemAccountType;;


  const treasuryAddress  = address(message.accountKeys.at(instruction.accounts[3]!)!)

  const treasuryAccount: PdaTypes.treasuryType = await GenericPda("treasuryPda",treasuryAddress) as  PdaTypes.treasuryType
  
  

  const thresholdAddress = address(message.accountKeys.at(instruction.accounts[2]!)!)

  const thresholdAccount: PdaTypes.thresholdType = await GenericPda("threshold",thresholdAddress) as  PdaTypes.thresholdType


  const dividendAddress = address(message.accountKeys.at(instruction.accounts[6]!)!)

  const dividendAccount:PdaTypes.dividendType = await GenericPda("dividendPda",dividendAddress) as PdaTypes.dividendType 



  const safetyAddress = address(message.accountKeys.at(instruction.accounts[5]!)!)

  const safetyAccount: PdaTypes.safetyType = await GenericPda("safetyPda",safetyAddress) as  PdaTypes.safetyType;


  const reinvestmentAddress = address(message.accountKeys.at(instruction.accounts[4]!)!)

  const reinvestmentAccount: PdaTypes.ReinvestmentType = await GenericPda("reinvestmentPda",reinvestmentAddress) as  PdaTypes.ReinvestmentType;
